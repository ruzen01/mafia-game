...
<div class="container mx-auto py-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 justify-items-center">
        ...
        <div 
            class="w-48 h-64 bg-white rounded-xl shadow-lg border-2 border-zinc-300 hover:shadow-xl hover:scale-105 hover:border-blue-400 transition-all duration-300 cursor-pointer relative overflow-hidden group"
            style="animation-delay: {{ $index * 0.1 }}s"
        >
            <!-- Верхний угол (как у карты) -->
            <div class="absolute top-2 left-2 w-8 h-8 bg-red-500 rounded-sm flex items-center justify-center">
                <span class="text-white text-xs font-bold">🃏</span>
            </div>

            <!-- Фото игрока или заглушка -->
            <div class="w-full h-32 bg-gradient-to-br from-zinc-100 to-zinc-200 flex items-center justify-center overflow-hidden">
            ...
            <!-- Имя игрока -->
            <div class="p-3 flex flex-col items-center h-24 justify-center bg-zinc-50">
                <a href="{{ route('players.show', $player->id) }}" class="font-bold text-center text-zinc-800 hover:text-blue-600 line-clamp-2 leading-tight transition-colors">
                    {{ $player->name }}
                </a>
            </div>
