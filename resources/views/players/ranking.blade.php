        ...
        <table class="table-auto border-collapse border border-gray-400 w-full">
            ...
            <tbody>
                ...
                ">
                    ...
                    <!-- Имя игрока — с глянцевым блоком для топ-3 -->
                    <td class="border border-gray-300 w-full px-4 py-1 text-left">
                        <a href="{{ route('players.show', $player->id) }}" 
                           class="
                                @if($loop->iteration <= 10) font-semibold @endif
                                @if($loop->iteration == 1)
                                    px-3 py-1 rounded-md bg-gradient-to-r from-yellow-300 to-yellow-500 text-yellow-900 shadow-sm hover:from-yellow-400 hover:to-yellow-600 transition-all duration-200 font-bold no-underline
                                @elseif($loop->iteration == 2)
                                    px-3 py-1 rounded-md bg-gradient-to-r from-gray-200 to-gray-300 text-gray-800 shadow-sm hover:from-gray-300 hover:to-gray-400 transition-all duration-200 font-bold no-underline
                                @elseif($loop->iteration == 3)
                                    px-3 py-1 rounded-md bg-gradient-to-r from-orange-200 to-orange-400 text-orange-900 shadow-sm hover:from-orange-300 hover:to-orange-500 transition-all duration-200 font-bold no-underline
                                @else
                                    hover:text-blue-600
                                @endif
                           ">
