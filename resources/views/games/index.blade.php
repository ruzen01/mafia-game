...
<div class="container mx-auto py-6 px-2" x-data="{ openModal: false, game: null }">
    ...
    <!-- Ключевая обертка: overflow-x-auto гарантирует, что таблица НИКОГДА не выйдет за границы контейнера -->
    <div class="overflow-x-auto rounded-lg shadow-lg bg-zinc-200">
        <table class="table-fixed border-collapse w-full text-xs">
            <thead class="bg-zinc-700 text-zinc-100 uppercase text-xs font-semibold">
                <tr>
                    <th class="border border-zinc-500 w-24 px-2 py-2 text-center">Дата</th>
                    <th class="border border-zinc-500 w-48 px-2 py-2 text-left">Имя игры</th>
                    <th class="border border-zinc-500 w-12 px-2 py-2 text-center">№</th>
                    <th class="border border-zinc-500 w-32 px-2 py-2 text-center">Ведущий</th>
                    <th class="border border-zinc-500 w-16 px-2 py-2 text-center">Сезон</th>
                    <th class="border border-zinc-500 w-32 px-2 py-2 text-center">Победитель</th>
                    <th class="border border-zinc-500 w-20 px-2 py-2 text-center">Игроки</th>
                    @can('update', App\Models\Game::class)
                    <th class="border border-zinc-500 w-36 px-2 py-2 text-center">Действия</th>
                    @endcan
                </tr>
            </thead>
            <tbody class="divide-y divide-zinc-500">
                ...
                <tr class="bg-zinc-200 animate-fade-in" style="animation-delay: {{ $loop->iteration * 0.1 }}s">
                    <td class="border border-zinc-500 w-24 px-2 py-2 text-center text-zinc-700 whitespace-nowrap">
                        {{ \Carbon\Carbon::parse($game->date)->format('d.m.Y') }}
                    </td>
                    <td class="border border-zinc-500 w-48 px-2 py-2 text-left">
                    ...
                    <td class="border border-zinc-500 w-12 px-2 py-2 text-center text-zinc-700">
                        {{ $game->game_number }}
                    </td>
                    <td class="border border-zinc-500 w-32 px-2 py-2 text-center text-zinc-700 truncate" title="{{ $game->host_name }}">
                        {{ $game->host_name }}
                    </td>
                    <td class="border border-zinc-500 w-16 px-2 py-2 text-center text-zinc-700">
                        {{ $game->season }}
                    </td>
                    <td class="border border-zinc-500 w-32 px-2 py-2 text-center text-zinc-700 truncate" title="{{ $game->winner }}">
                        {{ $game->winner }}
                    </td>
                    <td class="border border-zinc-500 w-20 px-2 py-2 text-center">
                    ...
                    @can('update', [$game])
                    <td class="border border-zinc-500 w-36 px-2 py-2 text-center space-y-1">
