    ...
    <form action="{{ route('games.update', $game->id) }}" method="POST">
        ...
        <div class="flex flex-wrap gap-4 mb-4 w-full">
            <div class="flex-1">
                ...
                <input type="date" name="date" id="date" value="{{ $game->date }}" class="border rounded py-2 px-3 w-full h-10 text-gray-900" required>
            </div>
            <div class="flex-1">
        ...
        <div class="flex flex-wrap gap-4 mb-4 w-full">
            <div class="flex-1">
                ...
                <input type="text" name="name" id="name" value="{{ $game->name }}" class="border rounded py-2 px-3 w-full h-10 text-gray-900" required>
            </div>
            <div class="flex-1">
                <label for="host_name" class="block text-sm font-medium">Ведущий:</label>
                <input type="text" name="host_name" id="host_name" value="{{ $game->host_name }}" class="border rounded w-full py-2 px-3 h-10 text-gray-900" required>
            </div>
            <div class="flex-1">
            ...
            <div id="players-list">
                @foreach($game->players as $player)
                    <div class="player-row flex flex-wrap gap-2 mb-2" data-player-id="{{ $player->id }}">
                        <select name="players[]" class="border rounded py-2 px-3 flex-1 h-10 text-gray-900">
                        ...
                        <select name="roles[]" class="border rounded py-2 px-3 ml-2 flex-1 h-10 text-gray-900">
                        ...
                        <input type="number" name="leader_scores[]" value="{{ $player->pivot->leader_score }}" placeholder="Баллы" class="border rounded py-2 px-2 ml-2 w-24 h-10 text-gray-900">
                        <input type="text" name="comments[]" value="{{ $player->pivot->comment }}" placeholder="Комментарий" class="border rounded py-2 px-3 ml-2 flex-1 h-10 text-gray-900">
...
<script>
    document.getElementById('add-player-row').addEventListener('click', function() {
        ...
        newRow.innerHTML = `
            ...
            <input type="number" name="leader_scores[]" placeholder="Баллы" class="border rounded py-2 px-2 ml-2 w-24 h-10 text-gray-900">
            <input type="text" name="comments[]" placeholder="Комментарий" class="border rounded py-2 px-3 ml-2 flex-1 h-10 text-gray-900">
