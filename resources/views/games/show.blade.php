        ...
        <table class="table-fixed border-collapse w-full bg-zinc-200 text-xs sm:text-sm">
            <thead class="bg-zinc-700 text-zinc-100 uppercase text-xs font-semibold">
                <tr>
                    ...
                    <th class="border border-zinc-500 w-12 px-1 py-2 text-center text-slate-300">Сезон</th>
                    <th class="border border-zinc-500 w-24 px-1 py-2 text-center text-green-300">Победитель</th>
                    <th class="border border-zinc-500 w-16 px-1 py-2 text-center text-blue-300">Игроки</th>
                    @canany(['update', 'delete'], App\Models\Game::class)
                    <th class="border border-zinc-500 w-20 px-1 py-2 text-center">Действия</th>
                    @endcanany
                </tr>
            </thead>
            <tbody class="divide-y divide-zinc-500">
                ...
                <tr class="bg-zinc-200 animate-fade-in" style="animation-delay: {{ $loop->iteration * 0.1 }}s">
                    <td class="border border-zinc-500 w-20 px-1 py-1 text-center text-zinc-700 whitespace-nowrap">
                    ...
                    <td class="border border-zinc-500 w-12 px-1 py-1 text-center text-slate-700">
                        {{ $game->season }}
                    </td>
                    <td class="border border-zinc-500 w-24 px-1 py-1 text-center text-green-700 truncate" title="{{ $game->winner }}">
                        {{ $game->winner }}
                    </td>
                                    ...
                                    @foreach($game->players as $player)
                                        {
                                            ...
                                            role: '{{ addslashes($roles[$player->pivot->role_id]->name ?? '—') }}',
                                            best_player: {{ $player->pivot->best_player ? 'true' : 'false' }},
                                            first_victim: {{ $player->pivot->first_victim ? 'true' : 'false' }},
                                            additional_score: {{ $player->pivot->additional_score ? 'true' : 'false' }}
                                        }@if(!$loop->last),@endif
                                    @endforeach
                                ]
                            }"
                    ...
                    @canany(['update', 'delete'], App\Models\Game::class)
                    <td class="border border-zinc-500 w-20 px-1 py-1 text-center">
                        <div class="flex justify-center space-x-1">
                            @can('update', $game)
                            <a href="{{ route('games.edit', $game->id) }}" 
                               class="bg-yellow-500 text-white text-[10px] py-0.5 px-1.5 rounded hover:bg-yellow-600 transition whitespace-nowrap flex items-center justify-center w-6 h-6"
                               title="Редактировать">
                            ...
                            <form action="{{ route('games.destroy', $game->id) }}" method="POST" class="inline-block">
                                ...
                                @method('DELETE')
                                <button type="submit" 
                                        class="bg-red-500 text-white text-[10px] py-0.5 px-1.5 rounded hover:bg-red-600 transition whitespace-nowrap flex items-center justify-center w-6 h-6"
